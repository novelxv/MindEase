...
const ChatDetailsScreen = ({ route, navigation }) => {
    ...
    const [messages, setMessages] = useState(chat.bubbleChats || []);
    ...
    return (
        <SafeAreaView style={globalStyles.container}>
            ...
            />
            <ScrollView showsVerticalScrollIndicator={false} style={styles.chatContainer} ref={scrollViewRef} onScroll={handleScroll} scrollEventThrottle={16}>
                {messages.map((message, index) => (
                <View key={index} style={message.sender === 'minnie' ? styles.minnieMessageContainer : null}>
                    {message.sender === 'minnie' && (
                        <View style={styles.profileContainer}>
                            <Image
                                source={require('../assets/Minnie-Profile.png')}
